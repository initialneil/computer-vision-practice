...
namespace surfelwarp {
	...
	public:
		...
		__host__ void Download(std::vector<T>& h_vec, cudaStream_t stream = 0) const {
			h_vec.resize(Size());
			cudaSafeCall(cudaMemcpyAsync(h_vec.data(), m_array, Size() * sizeof(T), cudaMemcpyDeviceToHost, stream));
		}
	};
