...
namespace surfelwarp {
	...
	public:
		...
		//The swap method
		void swap(DeviceBufferArray<T>& other) {
		...
		void ResizeArrayOrException(size_t size) {
			if (size > m_buffer.size()) {
				...
				printf("[DeviceBufferArray][ERROR] The pre-allocated buffer is not enough\n");
			}
		...
		void upload(const std::vector<T>& data) {
			int num = data.size();
			AllocateBuffer(num);
			ResizeArray(num, false);
			cudaSafeCall(cudaMemcpy(m_buffer.ptr(), (void*)data.data(), sizeof(T) * num, cudaMemcpyHostToDevice));
		}
		...
		void upload(const std::vector<T>& data, cudaStream_t stream) {
			...
			AllocateBuffer(num);
			ResizeArray(num, false);
			cudaSafeCall(cudaMemcpyAsync(m_buffer.ptr(), (void*)data.data(), sizeof(T) * num,
